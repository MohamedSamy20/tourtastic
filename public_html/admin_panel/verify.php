...
if (isset($_POST["verify"])) {
    ...
    if (isset($envatoRes->item->name)) {
        ...
        if ($data["item_id"] != $envatoItemId) {
            $result["msg"] =
                '<div class="alert alert-danger">Whoops! The purchase code provided is for a different item!</div>';
        } else {
            $result["msg"] =
                '<div class="alert alert-success">License Found!</div>';
            $result["start"] = '<table class="table table-hover">';
            $result["purchase_code"] =
                "<tr><td>Purchase Code</td><td>:</td><td>" .
                $product_code .
                "</td>";
            $result["item_id"] =
                "<tr><td>Item ID</td><td>:</td><td>" .
                $data["item_id"] .
                "</td>";
            $result["item_name"] =
                "<tr><td>Item Name</td><td>:</td><td>" .
                $data["item_name"] .
                "</td>";
            $result["buyer"] =
                "<tr><td>Buyer</td><td>:</td><td>" . $data["buyer"] . "</td>";
            $result["license"] =
                "<tr><td>License</td><td>:</td><td>" .
                $data["license"] .
                "</td>";
            $result["purchase_date"] =
                "<tr><td>Purchase Date</td><td>:</td><td>" .
                $data["purchase_date"] .
                "</td>";
            $result["purchase_count"] =
                "<tr><td>Purchase Count</td><td>:</td><td>" .
                $data["purchase_count"] .
                "</td>";
            $result["end"] = "</table>";
            ...
            $result["show_button"] =
                '<button type="submit" name="submit" class="button button-rounded btn-block waves-effect waves-float">SUBMIT</button>';
        }
    } else {
        $result["msg"] =
        ...
        <nav class="navbar">
            <div class="container-fluid">
                <div class="navbar-header">
                    <a class="navbar-brand" href="verify.php">
                                    ...
                                    <div class="col-sm-12">
                                        <center>
                                            ...
                                            <div class="col-sm-12">
                                                ...
                                                <br>
                                                <a href="https://codecanyon.net/item/webdroidx-android-webview-app-with-admin-panel/46734998" target="_blank"><b>Don't Have Purchase Code? I Want to Purchase it first.</b></a>
                                            </div>
                                        </center>
                                    </div>
                                </div>
