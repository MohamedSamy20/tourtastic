...
Route::group(['prefix'=>config('flight.flight_route_prefix')],function(){
    Route::get('/','FlightController@index')->name('flight.index'); // Index page
    Route::get('/search','FlightController@search')->name('flight.search'); // Search
    Route::post('getData/{id}',"FlightController@getData")->name('flight.getData');

    //Seeru API
    Route::post('/booking', 'FlightController@booking')->name('flight.booking');
    Route::get('/checkout', 'FlightController@checkout')->name('flight.checkout');
    Route::post('/complete-booking', 'FlightController@completeBooking')->name('flight.complete_booking');
    Route::get('/booking-confirmation/{booking_id}', 'FlightController@bookingConfirmation')->name('flight.booking.confirmation');
